...
import { flattenIssues, getIssueComparator, getIssueTypeTitle, getTableForType } from './util/util.ts';
  ...
  for (const [reportType, isReportType] of Object.entries(report) as Entries<typeof report>) {
    if (isReportType) {
      ...
      if (issuesForType.length > 0) {
        ...
        console.log(getTableForType(issuesForType, cwd, { isUseColors: false, comparator }).toString());
        console.log('```\n\n</details>\n');
      }
    }
  }
